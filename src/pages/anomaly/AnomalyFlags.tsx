...
function plainExplanation(
  ...
  switch ((pattern ?? "").toUpperCase()) {
    case "MONDAY_SICK":
      return `${firstName} has had a noticeable number of sick or emergency leave requests falling on Mondays. This has been flagged for a compassionate review — it may reflect a genuine recurring health issue, or it may be worth a brief conversation to understand the pattern.`
    case "SHORT_BURST":
    ...
    case "HOLIDAY_CLUSTER":
      return `${firstName}'s leave requests frequently land just before or after weekends and public holidays. This has been flagged so the HOD can confirm that coverage is being properly managed during these times.`
    case "APPRAISAL_AVOIDANCE":
      return `${firstName}'s leave history shows a pattern of taking leave around key appraisal or performance review periods. This may be coincidental, but it is helpful for HR to be aware when planning review schedules.`
    case "OVERLAP_PATTERN":
      return `${firstName} has been on leave more frequently than most colleagues in the same department recently. This has been flagged to ensure team coverage is adequate and workload fairly managed.`
    default:
      return `${firstName}'s recent leave pattern has been flagged for review because it differs noticeably from the rest of the department. This is an advisory flag — it does not reflect any breach of policy.`
  }
}
...
export function AnomalyFlags() {
  ...
  const dismissMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Flag dismissed."); queryClient.invalidateQueries({ queryKey: ["anomaly-flags"] }) },
    onError: (e) => toast.error(getApiErrorMessage(e)),
  })

  const resolveMutation = useMutation({
    mutationFn: anomalyApi.resolveFlag,
    onSuccess: () => { toast.success("Flag resolved."); queryClient.invalidateQueries({ queryKey: ["anomaly-flags"] }) },
    onError: (e) => toast.error(getApiErrorMessage(e)),
  })
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      <div>
        <h2 style={{ fontSize: "20px", marginBottom: "4px" }}>Anomaly Flags</h2>
        <p style={{ fontSize: "13px", color: "var(--text-muted)" }}>
          Active flags from the weekly leave pattern review.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <div style={{ textAlign: "center", padding: "48px", color: "var(--status-approved)" }}>
            ...
            <div style={{ fontSize: "14px", fontWeight: 600 }}>All clear — no open flags</div>
          </div>
        </Card>
      ) : (
        <>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "10px" }}>
                      Flagged: {flag.createdAt
                        ? new Date(flag.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })
                        : "—"}
                    </div>
                  </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "12px", color: "var(--text-muted)" }}>
            <span>
              Showing {page * pageSize + 1}–{Math.min((page + 1) * pageSize, filteredFlags.length)} of {filteredFlags.length} flag{filteredFlags.length !== 1 ? "s" : ""}
            </span>
